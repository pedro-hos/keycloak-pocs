...
function authenticated() {
	...
	$('#_campos').show();
	$('#message').html("<small>Olá " + keycloak.tokenParsed['preferred_username'] + "</small>");
}
		...
		$.each(livros, function(i, livro) {
			...
			livroHtml.push("<tr>");
				...
				livroHtml.push("<td>" + livro.id + "</td>" +
						       ...
						       "<td>" + livro.autor + "</td>");
				livroHtml.push("<td>" +
									"<button class=\"btn btn-link\" onclick='apagar(" + livro.id + ")'><i class=\"icon-remove-circle\"></i></button> " +
									"<button class=\"btn btn-link\" onclick='editar(" + i + ")'><i class=\"icon-edit\"></i></button>" +
							  "</td>");
